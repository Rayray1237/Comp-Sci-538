...
interp :: Expr -> Int
-- <FILL-IN>
interp (Op op x y) = (interpBinOp op) (interp x) ( interp y)
interp (Lit x) = x
...
-- <FILL-IN>
simplifyZero (Op Plus (Lit 0) y) = y
simplifyZero (Op Plus x (Lit 0)) = x
simplifyZero z = z
...
decompileInstr (IOp b) (y : x : rest) = Just (Op b x y : rest)
