...
const ls_size = ls => ls.reduce((acc, el) => acc + 1, 0);
...
const sum_sqrs = ls => ls.reduce((acc, el) => acc + el*el, 0);
...
const ls_min = ls => {if (!ls.length) return undefined;
return ls.reduce((a, b) => Math.min(a, b));
};
const ls_max = ls => {
  if (!ls.length) return undefined;
  return ls.reduce((a, b) => Math.max(a, b));
};
